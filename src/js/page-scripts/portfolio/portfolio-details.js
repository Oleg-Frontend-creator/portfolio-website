...
function renderProject(project) {
    ...
    const techStackEl1 = document.querySelector('[data-project-techstack-1]');
    const techStackEl2 = document.querySelector('[data-project-techstack-2]');
    const difficultyEl1 = document.querySelector('[data-project-difficulty-1]');
    const difficultyEl2 = document.querySelector('[data-project-difficulty-2]');
    ...
    difficultyEl1.textContent = project.difficultyInProjectText;
    difficultyEl2.textContent = project.difficultyInProjectText;

    // Технологии
    techStackEl1.innerHTML = '';
    (project.techStack || []).forEach((tech) => {
        const li = document.createElement('li');
        li.textContent = tech;
        techStackEl1.appendChild(li);
    });
    techStackEl2.innerHTML = '';
    (project.techStack || []).forEach((tech) => {
        const li = document.createElement('li');
        li.textContent = tech;
        techStackEl2.appendChild(li);
    });
...
function renderPortfolioNav(currentId) {
    const navElement = document.querySelector('[data-portfolio-nav]');
    if (!navElement) return;

    const index = portfolioData.findIndex(item => item.id === currentId);
    const prev = portfolioData[index - 1];
    const next = portfolioData[index + 1];

    let html = '';

    if (prev) {
        html += `
            <a class="nav__btn prev" href="portfolio-details.html?id=${prev.id}">
    ...
    if (next) {
        html += `
            <a class="nav__btn next" href="portfolio-details.html?id=${next.id}">
                Следующий

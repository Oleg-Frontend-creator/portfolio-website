...
function renderService(service) {
    ...
    const serviceSubtitleEl = document.querySelector('[data-service-subtitle]');
    const cardsEl           = document.querySelector('[data-service-cards]');
    ...
    // "Подойдёт вам, если..."
    if (serviceSubtitleEl) serviceSubtitleEl.textContent = service.fitIntro ?? '';
    ...
    // FAQ
    if (faqEl) {
        faqEl.innerHTML = '<header class="box">FAQ</header>';
...
function renderServiceNav(currentId) {
    ...
    const index = serviceData.findIndex(item => item.id === currentId);
    const prev = serviceData[index - 1];
    const next = serviceData[index + 1];
